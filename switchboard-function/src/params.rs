...
impl ContainerParams {
    pub fn decode(container_params: &Vec<u8>) -> std::result::Result<Self, SwitchboardClientError> {
        ...
        let mut min_guess: u32 = 0;
        let mut max_guess: u32 = 0;
        ...
        for env_pair in params.split(',') {
            ...
            if pair.len() == 2 {
                match pair[0] {
                    ...
                    "MIN_GUESS" => min_guess = pair[1].parse::<u32>().unwrap(),
                    "MAX_GUESS" => max_guess = pair[1].parse::<u32>().unwrap(),
...
mod tests {
    ...
    fn test_params_decode() {
        ...
        assert_eq!(params.min_guess, 1);
        assert_eq!(params.max_guess, 6);
